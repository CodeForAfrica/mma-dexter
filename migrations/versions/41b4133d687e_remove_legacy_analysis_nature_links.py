...
def upgrade():
    ...
    op.drop_constraint('source_roles_ibfk_1', 'source_roles', type_='foreignkey')
...
def downgrade():
    ...
    op.create_foreign_key('source_roles_ibfk_1', 'source_roles', 'analysis_natures', ['analysis_nature_id'], ['id'])
    op.add_column('issues', sa.Column('analysis_nature_id', mysql.INTEGER(display_width=11), server_default=sa.text("'1'"), autoincrement=False, nullable=True))
    op.create_foreign_key('issues_ibfk_1', 'issues', 'analysis_natures', ['analysis_nature_id'], ['id'])
    ### end Alembic commands ###
